...
public class VendasImpl implements VendasQueries {
	...
	public Long quantidadePedidosPendentes() {
		Optional<Long> optional = Optional.ofNullable(
				manager.createQuery("select count(*) from Venda where  status = :status", Long.class)
	...
	public BigDecimal valorDia() {
		Optional<BigDecimal> optional = Optional.ofNullable(
				manager.createQuery("select sum(valorTotal) from Venda where day(dataCriacao) = :dia and status = :status", BigDecimal.class)
					.setParameter("dia", MonthDay.now().getDayOfMonth())
					.setParameter("status", StatusVenda.EMITIDA)
					.getSingleResult());
		return optional.orElse(BigDecimal.ZERO);
	}
